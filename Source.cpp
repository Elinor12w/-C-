...
int findAccount(Account* bank, int size) { // Finding an account
	int number, code;
...
void printTransaction(Account a, ACTION ac, Clock& c) {//Print the executed transaction
	cout << c << "\t";
	switch (ac) {
...
void getBalance(Account* bank, int size, Clock& c) { //Update the balance
...
void cashWithdraw(Account* bank, int size, Clock& c) {//Cash withdrawal
	int i = findAccount(bank, size);
	float amount;
...
void checkAccount(Account bank[], int i) {//Checking the integrity of the account
	for (int j = 0; j < i; j++)
